...
def runExperiment():
    ...
    if cfg['resume_mode'] == 1:
        ...
        logger = make_logger('output/runs/train_{}'.format(cfg['model_tag']))
    if last_epoch == 1:
        assist.initialize(dataset, organization, metric, logger, 0)
    for epoch in range(last_epoch, cfg[cfg['global']]['num_epochs'] + 1):
        data_loader = assist.make_dataset(dataset, epoch)
        ...
        test(data_loader, organization, metric, logger, epoch)
        result = {'cfg': cfg, 'epoch': epoch + 1, 'assist': assist, 'organization': organization,
                  'logger': logger}
...
def test(data_loader, organization, metric, logger, epoch):
    with torch.no_grad():
